  ...
  <section class="admin">
    ...
    <template v-else>
      ...
      <div class="admin__totals">
        <div class="card">
          <span class="card__label">Wallets actifs qualifiés</span>
          <strong class="card__value">{{ overview.totals.activeUsers ?? "—" }}</strong>
          <ul class="segments">
            <li v-if="overview.totals.activeUsers !== null">Wallet actif + tx SourceTag réussie + NFT First Trade</li>
            <li v-else class="admin__warning">Vérification XRPL Mainnet indisponible</li>
          </ul>
        </div>
      ...
      <section class="wallet-manager">
        ...
        <div class="wallet-manager__toolbar">
          <select v-model="batchBadgeCode" :disabled="loading" aria-label="Badge individuel">
