...
export interface AdminTotals {
  ...
  /** Wallets actifs avec une tx Tide réussie et un NFT First Trade détenu. */
...
export interface AdminServiceDeps {
  ...
  /** Lecture Mainnet des NFT et de l'attribution pour les wallets candidats. */
...
export class AdminService {
  ...
  async overview(prices: PriceMap): Promise<AdminOverview> {
    ...
    const [activeUsers, fundedWalletsWithNft] = await Promise.all([
      this.countActiveUsers(wallets),
      this.countFundedWalletsWithNft(wallets),
    ]);
  ...
  private async countActiveUsers(
    ...
    if (reader?.addressesWithFirstTradeNftAndTaggedTx === undefined) return null;
    const candidates = wallets.filter((wallet) =>
      wallet.address !== null && (
        (wallet.kind === "paper" && wallet.status === "funded") ||
        (wallet.kind === "external" && wallet.live)
      ),
    );
    const addresses = [...new Set(candidates.flatMap((wallet) =>
      wallet.address === null ? [] : [wallet.address],
    ))];
    if (addresses.length === 0) return 0;
    try {
      const qualified = await reader.addressesWithFirstTradeNftAndTaggedTx(addresses);
      return new Set(
        candidates
          .filter((wallet) => wallet.address !== null && qualified.has(wallet.address))
          .map((wallet) => wallet.address),
      ).size;
    } catch (error) {
      console.error("[admin] lecture activité First Trade Mainnet échouée:", error);
      return null;
    }
  }
