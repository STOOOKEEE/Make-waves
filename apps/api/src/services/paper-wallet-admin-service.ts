...
import { extractTaggedTxs, type NftIssuer, type NftIssueResult } from "@tide/xrpl";
...
const FIRST_TRADE_TAXON = 1;

function isMissingXrplAccount(error: unknown): boolean {
  if (!(error instanceof Error) || !("data" in error)) return false;
  const data = error.data;
  return typeof data === "object" && data !== null && "error" in data && data.error === "actNotFound";
}
...
export interface PaperWalletAdminServiceDeps {
  ...
  /** Anciennes lignes wallet 2, conservées uniquement pour la récupération opérateur. */
...
export class PaperWalletAdminService {
  ...
  async grantBadge(userId: string, badgeCode: string): Promise<AdminNftGrantResult> {
    ...
    if (wallet?.status !== "funded") throw new Error("Wallet déjà financé requis");
...
export class XrplPaperWalletAdminGateway implements PaperWalletAdminGateway {
  ...
  /**
   * Vérifie les trois conditions du compteur « users actifs » sur Mainnet :
   * wallet existant avec un NFT First Trade et au moins une tx Tide réussie.
   */
  async addressesWithFirstTradeNftAndTaggedTx(
    ...
    const worker = async (): Promise<void> => {
      while (cursor < unique.length) {
        ...
        try {
          ...
          if (!nfts.result.account_nfts.some((nft) => nft.NFTokenTaxon === FIRST_TRADE_TAXON)) {
            continue;
          }
          if (await this.hasTaggedTransaction(client, address)) matches.add(address);
        } catch (error) {
          // Compte supprimé ou jamais activé = non qualifié, pas une panne XRPL.
          if (!isMissingXrplAccount(error)) throw error;
        }
      }
    };
    try {
  ...
  private async hasTaggedTransaction(client: Client, address: string): Promise<boolean> {
    ...
    do {
      ...
      const extracted = extractTaggedTxs(page.result.transactions, this.sourceTag);
      if (extracted.txs.some((tx) => tx.account === address)) return true;
