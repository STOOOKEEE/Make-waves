...
- **T4** route `GET /admin/overview` (`apps/api/src/http/server.ts` + montage conditionnel dans `main.ts`) : garde par header `x-admin-token`, comparaison directe côté serveur (constant-time non requis à ce niveau de menace), **absente de `main.ts` sans `TIDE_ADMIN_TOKEN`** → 404 (pas juste 401 — la route n'existe pas si non configurée, zéro surface par défaut).
