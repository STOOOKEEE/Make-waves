...
**MàJ (13/07, post-livraison) — accept taggé.** L'`NFTokenAcceptOffer` (tx signé par le user) porte désormais le **SourceTag Tide** : c'est le tx d'un humain → il compte pour l'attribution (compte actif). Construit **côté serveur** (`buildBadgeAcceptOffer` dans `BadgeService.claim`) et renvoyé prêt-à-signer dans la réponse de claim (`acceptTx`) ; le front le soumet tel quel via GemWallet (plus de tx fabriqué au front, pas de bundle `xrpl`). Ripple : `claimBadge` renvoie `{sellOfferId, nftTokenId, acceptTx}` (client `ClaimBadgeResult`), `useBadges`/`useWallet.signBadgeAccept` prennent l'`acceptTx`. Tests des ids passés en 64-hex réalistes (le builder valide le format).

**Reste (dette tracée).** (1) **Xaman-claim** non câblé (chemin Gem uniquement) → route `/sign/badge-accept` (mirror `/sign/live-offer`) à ajouter. (2) Confirmation du claim = confiance au retour de signature (MVP) ; durcissement via l'indexeur F4. (3) Assets d'images des badges (`/badges/*.svg`) à produire. (4) **Vérif navigateur runtime** par Armand (claim GemWallet réel → NFT dans le wallet) — non fait ici. (5) Le **claim on-chain** reste OFF tant que `TIDE_NFT_ISSUER_SEED` n'est pas fourni (l'affichage off-chain, lui, est désormais toujours actif — cf. entrée 14/07).

## 2026-07-14 — Déploiement prod des badges + découplage affichage/issuer + merge main

**Quoi.** Merge de `feat/roadmap-lots` → `main` (PR #5, 19 commits) puis **déploiement en production** du lot (chat DeepSeek + badges + scaffolding). Puis correctif produit : l'**affichage des badges off-chain** est découplé de l'issuer NFT (marche pour tous, sans aucune config on-chain). **Vérifié live** : `api.tidetrade.xyz` + `tidetrade.xyz` servent le nouveau code.

**Pourquoi.** Armand voulait tout pousser en prod. Puis, une fois déployé, constat : la route badges était gated derrière l'issuer → sans seed, la grille Portfolio ne chargeait pas, ce qui **contredit l'intention du brainstorm** (« badges off-chain gratuits pour TOUS » = le chiffre d'users). Découplage nécessaire.

**Cheminement (infra découverte à chaud, serveur = Tailscale `100.113.76.101`, repo `~/tide-app`).**
- **Le serveur n'est pas un dépôt git** (déployé par copie de fichiers). `git pull` impossible → synchro par **`git archive HEAD | ssh serveur 'tar -xf -'`** : n'envoie que les fichiers **suivis** (secrets `deploy/api.env` + `.db` + `node_modules` **auto-exclus** car gitignorés ; vérifié : seuls des `.env.example` dans l'archive). L'extraction n'écrase que les sources, laisse `api.env`/`.db` intacts.
- **rsync écarté** : présent en local mais **absent côté serveur** (rsync exige les 2 bouts).
- **Front = nginx servant des fichiers statiques** depuis `deploy/dist/` (bind-mount `./dist`, chemins compose relatifs à `deploy/`), **pas** un build Docker. Donc rebuild ≠ redéploie le front. Front **buildé en local** avec `VITE_API_BASE=https://api.tidetrade.xyz` (le front appelle l'API cross-origin), puis `deploy/dist/` remplacé via tar over ssh (`find -mindepth 1 -delete` pour garder l'inode du bind-mount + `COPYFILE_DISABLE=1` contre les fichiers `._` macOS).
- **API = build Docker** (`deploy/Dockerfile.api`, context repo root) : `docker compose -f deploy/docker-compose.prod.yml up -d --build api`. Build en échec = anciens conteneurs conservés (pas de downtime).
- Conteneurs : `tide-api` (Docker build), `tide-web` (nginx:alpine + `deploy/dist`), `tide-app-tunnel` (cloudflared → domaines).

**Découplage (code).** `BadgeService` : `issuer`/`sourceTag`/`metadataBaseUrl` passés **optionnels**. `statusFor`/`confirmClaim` marchent sans issuer ; `claim` lève **`BadgeClaimUnavailableError` (503)** si pas d'issuer. Câblage : `BadgeService` monté dès qu'un `store` est fourni (plus besoin de l'issuer). Les routes `GET /accounts/:id/badges` + `/nft-metadata` marchent toujours ; `claim` renvoie 503 sans seed.

**Bugs & fix.**
1. **Grille badges 404 sans issuer** (root cause du découplage) → fix ci-dessus.
2. **`statusFor` 404 pour un compte inexistant.** `PaperService.ordersOf` lève `AccountNotFoundError` si le compte paper n'existe pas → un **nouveau visiteur** (0 ordre) recevait 404 au lieu de « 0 badge ». Fix : `BadgeService.fillCountOf` catch **spécifiquement** `AccountNotFoundError` (par `err.name`, sans coupler à la classe) → `fillCount = 0` ; toute autre erreur remonte (pas d'avalement). Vérifié live : `GET /accounts/brandnewuser/badges` → 200, 3 badges `earned:false`.

**Vérifié (live, prod).** `GET /nft-metadata/first_trade` 200 · `GET /accounts/<any>/badges` 200 (3 badges) · `POST /badges/:code/claim` **503** (pas d'issuer) · front `tidetrade.xyz` sert le bundle avec la grille badges · `tide-api` healthy. **820 tests, typecheck 8/8, lint 0.**

**Reste.** (1) **Configurer l'issuer** pour activer le claim on-chain : créer un **sous-compte issuer dédié** (fundé par le wallet-maître, ~1-2 XRP — **PAS** le seed du wallet-maître sur le serveur, blast radius), mettre son seed dans `deploy/api.env` (`TIDE_NFT_ISSUER_SEED`) + `XRPL_WSS_URL=wss://s1.ripple.com` + `TIDE_SOURCE_TAG`, puis restart api. (2) URL image des métadonnées sort en `http://` (devrait être `https://`) + assets `/badges/*.svg` inexistants (cosmétique). (3) Xaman-claim (Gem seul). (4) Vérif navigateur du flow claim réel. (5) Pas de CI : tout déploiement = `git archive`+`tar`+`docker compose up --build` manuels (candidat à un script `deploy/deploy.sh`).

## 2026-07-14 — Console admin en lecture seule (`/admin`, segmentation operator/agent/frontend) [6 tâches, spec → plan → TDD]

**Quoi.** Console de supervision **read-only** pour Armand : vue unifiée des comptes paper (users + agents IA), segmentés par origine, plus les wallets connus (agents live + prize pool). Spec [`docs/superpowers/specs/2026-07-14-admin-dashboard-design.md`], plan [`docs/superpowers/plans/2026-07-14-admin-dashboard.md`]. Livré en 6 commits :
- **T1** `AgentStore.list()` (InMemory + SQLite) — lister tous les agents, nécessaire pour l'agrégation.
- **T2** `AdminService` (`apps/api/src/services/admin-service.ts`) — `buildOverview()` combine `PaperService`/`LeaderboardService`/`AgentStore`/`AgentActionsStore` : `classify(userId)` segmente chaque compte en `operator` (allowlist) `> agent` (possède un agent) `> frontend` (reste), `buildTotals` (compte par segment + statuts agents + wallets), `buildWallets` (agents en mode live + prize pool de config, **pas de fetch on-chain** en v1). Précédence figée par un test dédié.
- **T3** env `TIDE_ADMIN_TOKEN` (secret de la route) + `TIDE_OPERATOR_USER_IDS` (CSV → allowlist operator), documentées dans `.env.example`.
- **T4** route `GET /admin/overview` (`apps/api/src/http/server.ts` + montage conditionnel dans `main.ts`) : garde par header `x-admin-token`, comparaison **timing-safe**, **absente de `main.ts` sans `TIDE_ADMIN_TOKEN`** → 404 (pas juste 401 — la route n'existe pas si non configurée, zéro surface par défaut).
- **T5** `@tide/client.adminOverview(token)` — GET avec le header, typé.
- **T6** front `useAdmin` (composable) + `AdminView.vue` à `#/admin` — saisie du token en session (pas persisté), cartes de totaux + tables users/agents/wallets, **aucune action d'écriture**, hors nav publique (URL directe uniquement).

**Pourquoi.** Armand pilote un mix de users réels (démo/proto) + agents IA (mandats autonomes) + son propre compte opérateur, sans vue consolidée pour distinguer qui est qui ni suivre l'équity/PnL global. Une console lecture-seule, gate-ée par token et OFF par défaut, donne cette visibilité sans risque (pas de bouton d'écriture, pas exposée si le token n'est pas positionné).

**Cheminement (alternatives écartées).** Segmentation par **allowlist + appartenance agent** plutôt qu'un champ `role` stocké en DB — zéro migration, zéro nouvelle colonne, la classification est un calcul pur à la lecture (`classify`), reclassable en changeant juste l'env. Précédence `operator > agent` : un ID à la fois dans l'allowlist et propriétaire d'un agent reste compté `operator` (c'est un choix explicite d'Armand, pas un compte agent générique). Wallets = **config statique** (agents live connus + prize pool), pas de scan on-chain — suffisant pour la v1, évite une dépendance réseau supplémentaire sur une vue de supervision. Petit accroc : la tâche 3 avait d'abord dupliqué la doc des variables dans `apps/api/.env.example` alors que le projet centralise dans le `.env.example` racine → **fix immédiat** (commit `6721e67`) : déplacé dans `.env.example` racine, `apps/api/.env.example` redevient vide de ces clés (pas de doublon de documentation).

**Bugs & fix.** Aucun bug fonctionnel (TDD, chaque tâche verte au commit). Le seul accroc est le doublon `.env.example` ci-dessus, corrigé dans la foulée.

**Vérifié (bout en bout, tâche 7).** `pnpm typecheck` 8/8 vert (une occurrence transitoire de `Cannot find name 'registerAuth'` sur `apps/api` en run parallélisé, non reproductible au run suivant — flake de build concurrent du monorepo sur le WIP `auth/` non lié à cette feature, pas une régression admin). `pnpm lint` 0 erreur. `pnpm test` **885/886** verts ; le seul échec (`packages/mcp/src/lib/api-client.test.ts`, assertion sur un header `Authorization: Bearer` absent) appartient au chantier WIP non commit *Sign-In with XRPL* (JWT bearer, fichiers non trackés `apps/api/src/auth/*` + modif de `packages/mcp/src/lib/api-client.ts`) — hors périmètre de cette tâche, non touché. Smoke runtime : sans header → **401** ; header invalide → **401** ; `x-admin-token: devtoken` correct → **200** + JSON `totals`/`users`/`agents`/`wallets` cohérent ; sans `TIDE_ADMIN_TOKEN` au boot → **404** (OFF par défaut confirmé).

## 2026-07-14 — Sécurité F1 : authentification + autorisation (Sign-In with XRPL) [8 tâches, plan → TDD, NON COMMIT]

**Quoi.** Fondation d'auth de l'API (audit sécurité, faille CRITIQUE F1 : **aucune authentification**, l'identité = l'adresse XRPL **publique** → n'importe qui lisait/agissait sur le compte de tout le monde). Livré (non commit, branche `feat/api-security-f1`) :
- `@tide/xrpl` `tx/message.ts` : `verifyMessageSignature` + `addressFromPublicKey` (adossés à `ripple-keypairs`).
- `apps/api/src/auth/` : `challenge-store` (nonces Gem, TTL 5 min, usage unique), `auth-service` (challenge/verifyGem/verifyXaman + JWT HS256 `sub=adresse`), `guard` (fonction pure `authorize` : table publique + règles de propriété `:userId`/body/query + agent/mandat).
- `http/server.ts` : `preHandler` global + routes `/auth/challenge` et `/auth/verify` ; câblage `app.ts`/`main.ts` (AuthService + resolvers, **secret requis au boot**) ; env `TIDE_SESSION_SECRET`/`TIDE_SESSION_TTL`.
- `@tide/client` : `setToken` (Bearer auto) + `authChallenge`/`authVerifyGem`/`authVerifyXaman`. Front : `useAuth` + branchement `useWallet` (Gem signe un challenge, Xaman réutilise le SignIn) + restore au boot (App.vue) + purge à la déconnexion.
- `@tide/mcp` `TideApiHttp` : `authToken` optionnel (Bearer) — la fabrique de chat agent minte un JWT du user propriétaire pour le self-HTTP.

**Pourquoi.** L'app garde des fonds (Live spot taggé, claim NFT, mandats agents). Sans auth, tout endpoint compte/agent/mandat était ouvert. F1 est le **prérequis** de F3 (filtrage SSE) et F4 (claim badge), et de la règle absolue « pas de Live agent / claim on-chain en prod avant F1+F2 ».

**Cheminement (décisions).** (1) **JWT Bearer** (pas cookie) : le front est cross-origin (`tidetrade.xyz`→`api.tidetrade.xyz`), Bearer stateless = plus simple qu'un cookie `SameSite=None; Secure`+CORS credentials. (2) **`jsonwebtoken`** plutôt que `@fastify/jwt` : le service reste testable sans Fastify (le `preHandler` fait déjà la logique custom). (3) **Message signé lie adresse+nonce** (reconstruit serveur, jamais choisi par le client) → une signature valide de sa propre clé ne peut pas usurper une autre adresse (`addressFromPublicKey === address` exigé). (4) **Self-HTTP agent-chat** : la fabrique passe `authToken = issueToken(userId)` → les appels internes passent le garde comme des appels du user (pas de backdoor IP/loopback ni secret interne). (5) **Connect GemWallet exige désormais une signature** (avant : `getAddress()` seul = adresse non prouvée, cœur de la faille). (6) **`/admin/overview` mis public** dans le garde JWT : il se protège seul via `x-admin-token` (opérateur), pas un JWT user — sinon la console admin (livrée en parallèle) cassait. (7) Nonce Gem **en mémoire** (déploiement mono-conteneur ; `ponytail:` SQLite si multi-instance).

**Bugs & fix.** (1) Test existant `apps/api/test/agent-chat-ctx.test.ts` : la fabrique a gagné un 4e arg `issueToken` → 2 appels du test mis à jour (stub). (2) **Smoke trompeur** : le premier boot curl-é tapait un **serveur concurrent résiduel** (`main.ts` PID 61808 sur `:3997`, ancien code sans auth → 404 sur `/auth/*` et 404 au lieu de 401) — un **autre agent tournait sur le même worktree**. Re-run sur un port propre (`:3901`) → public **200**, protégé sans token **401**, `/auth/challenge` émet le nonce, boot **refuse** sans `TIDE_SESSION_SECRET`.

**Vérifié.** **901 tests** verts (58 nouveaux), **typecheck 8/8**, **lint 0**, + smoke runtime ci-dessus.

**Reste / conséquences.** (a) **F2** (vérif signature du mandat) pas encore fait → pas de Live agent en prod. (b) **Prod ne boote plus sans `TIDE_SESSION_SECRET`** (voulu ; ajouté à `.env.example`). (c) **MCP externe** (`bin/tide-mcp.ts`) : routes comptes gardées → doit lire un token d'env (seam `authToken` posé, wiring restant). (d) `/metrics` exige maintenant un token (repli authentifié) — à rendre public si un affichage le lit. (e) **Worktree partagé avec un agent concurrent** (a committé le lot admin `/admin` sur cette même branche pendant F1) — coordination à surveiller. F1 non commit à ce stade.

## 2026-07-14 — Sécurité F2–F8 : mandat, SSE, claim, CORS, rate-limit, host header [TDD]

**Quoi.** Suite de l'audit après F1 (commit `6781ef7`) :
- **F3 (SSE cross-user)** : `/api/agents/events` était un bus global → tout client recevait chaque `agent_action`/`agent_killed`. Nouveau `sse/event-filter.ts` (filtre par propriétaire, cache par agent) ; la route s'auto-garde (token en **query `?token=`** car EventSource ne pose pas de header) et ne relaie que les events de l'utilisateur. Front : `readSessionToken()` partagé → EventSource (`?token=`) ET le fetch du chat agent (`Authorization`) — sinon F1 les cassait.
- **F4 (drain issuer NFT)** : le garde exige désormais `walletAddress === tokenAddress` sur `/badges/:code/claim` (mint uniquement vers l'adresse authentifiée). Bornage « pending » **déjà couvert** par l'idempotence existante (1 claim par (user,code), 3 badges max) → pas de compteur ajouté (YAGNI).
- **F2 (mandat auto-déclaré)** : `onSignCallback` acceptait n'importe quelle string. Désormais **Paper = activation par le propriétaire** (déjà gardé F1, fonds virtuels) ; **Live (agent `hasLiveAccount`) = preuve Xaman obligatoire** (`getPayloadStatus` : `signed && address === mandate.userId`), **fail-closed** sans XUMM. `MandateService` gagne un resolver `agentIsLive` ; `main.ts` adapte l'API Xaman réelle (`sign.api.get`) au shape mandat.
- **F6** : CORS `origin: deps.corsOrigin ?? true` (env `TIDE_CORS_ORIGIN`, CSV). **F8** : `/nft-metadata` utilise `TIDE_PUBLIC_BASE_URL`, plus le header `Host` (injection). **F7** : rate limiting.

**Cheminement (F7 — décision notable).** `@fastify/rate-limit` **écarté** : il exige `await register` AVANT les routes (son hook `onRoute`/global n'est actif qu'après le boot du plugin) ; or `buildServer` est **synchrone** et le rendre async casserait ~100 tests (`const app = buildServer(); app.inject()`). Prouvé au banc : `void app.register(...)` + routes sync → **0 limitation** (aucun header `x-ratelimit`), alors que `await register` avant les routes → OK. Choix : **limiteur fenêtre-fixe maison** (`http/rate-limiter.ts`, ~25 lignes, `Map<key,{count,resetAt}>`) posé en `onRequest` synchrone → borne globale par IP + borne stricte sur `/auth/*`. Dépendance `@fastify/rate-limit` retirée. *ponytail : pas d'éviction des clés (mémoire bornée par IP distinctes/fenêtre ; prune si ça explose).*

**Bugs & fix.** Le premier smoke F7 montrait 0×429 (plugin inactif, cf. ci-dessus) → remplacé par le limiteur maison, re-smoke : 15×200 puis 5×429 sur `/auth/challenge`. CORS `origin` type : `readonly string[]` refusé par `@fastify/cors` → `string | string[]`.
...
## 2026-09-09 — Tombola AirPods Max : page de campagne `#/giveaway` (front)

**Quoi.** Nouvelle page publique **`#/giveaway`**, bilingue FR/EN, atteignable **uniquement par le footer de la landing** (le footer ne vit que dans `LandingView.vue`, donc c'est effectivement la seule porte d'entrée) et volontairement absente des onglets de l'app-bar. Elle met en jeu une paire d'AirPods Max : hero produit, compte à rebours, trois étapes de participation, **compteur d'entrées personnel**, panneau « le lot », règlement complet dépliant, CTA final. Nouveaux fichiers : `apps/web/src/views/GiveawayView.vue` (~540 l.), `apps/web/src/composables/useGiveaway.ts`, `apps/web/src/data/giveaway.ts`, `apps/web/src/assets/giveaway/` (2 WebP + `CREDITS.md`). **1169 tests** (+19), typecheck 8/8, lint 0, build OK.

**Pourquoi.** Acquisition d'utilisateurs et croissance du compte X. La mécanique intéressante n'est pas le tirage, c'est la **pondération** : +1 créer un compte, +3 premier trade paper, +2 par ami parrainé — chaque entrée supplémentaire coûte un pas de plus dans le produit.

**Cheminement (les décisions qui ont structuré le résultat).**
- **Deux règles sur trois sont réelles sans une ligne de backend.** `+1 compte` se dérive de `useAccountAuth().signedIn` (identité Supabase, déjà en place) et `+3 premier trade` de `client.badges(userId)` → `first_trade.earned` (le mérite est déjà dérivé côté serveur par `badges/merit.ts`). Seul le **parrainage** manque de tout : aucune primitive de referral n'existe dans le repo (`ROADMAP.md:106` la liste encore comme à faire). Il est donc **affiché en `pending`** — « bientôt » — et **jamais compté**. Annoncer une règle qu'on ne sait pas mesurer serait un mensonge par omission ; l'annoncer en la marquant à venir est honnête et garde la page complète.
- **Une seule couture vers les données** : `useGiveaway.ts`. La vue ne parle jamais au client. Quand la route serveur existera, on remplace la source dans ce fichier et `GiveawayView.vue` ne bouge pas d'une ligne. Le contrat et le chemin de code complet sont dans [`docs/superpowers/specs/2026-09-09-giveaway-entries-design.md`](superpowers/specs/2026-09-09-giveaway-entries-design.md).
- **Aucun total public.** `growth/context/05-facts.md` interdit de publier nos chiffres de traction (« specific traction numbers (users, signups, conversion) »). Le seul compteur de la page est **personnel** — « tes entrées » — et un test échoue si un total global réapparaît.
- **Aucune session anonyme créée par une simple visite.** Les entrées se rattachent à un compte : tant qu'il n'y en a pas, il n'y a rien à lire. `resolveUserId()` ne réveille `ensurePaperSession()` que si le visiteur est déjà connecté. Vérifié au navigateur : un visiteur anonyme déclenche **zéro appel API**.
- **Pas de modèle 3D, et c'est délibéré.** `apps/web/package.json` n'a aucune dépendance de rendu et le repo pose la règle « canvas maison, aucune dépendance ». Le relief vient des primitives existantes : anneaux d'orbite CSS de `LearnView`, glow radial bleu de `LandingView`, léger flottement coupé sous `prefers-reduced-motion`.
- **Assets** : deux photos Unsplash (licence commerciale libre, sans attribution). Le héros est **rabattu sur le bleu de marque** en écrasant la teinte HSV sur une constante (163/255 ≈ `#4F6AFF`) plutôt qu'en la faisant tourner — une rotation fait dériver le dégradé vers le violet en passant par le rouge. Recette reproductible et vérification colorimétrique dans `apps/web/src/assets/giveaway/CREDITS.md`.
- **Balises OG globales laissées intactes.** `index.html:16-21` explique que la SPA en hash-routing n'a qu'un seul jeu de balises pour tout le site : y mettre une image AirPods transformerait la carte de partage de *tidetrade.xyz* entier en carte de tombola. Pour le post X, l'image s'attache nativement au tweet, ce qui donne de toute façon une meilleure distribution.
- **Conformité.** Règlement complet : éligibilité 18+ monde entier, gratuité sans obligation d'achat, dates, mode de tirage, prise de contact par l'e-mail du compte Tide, délai de sept jours. Plus les deux mentions obligatoires — Apple n'est ni organisateur ni sponsor ; l'opération n'est ni sponsorisée ni administrée par X. Un test vérifie leur présence, et qu'aucune formule interdite (`risk-free`, `sans risque`, `prize pool`, `cagnotte`, promesse de gain) n'apparaît dans les deux langues.
- **`useCountdown.ts` réparé plutôt que dupliqué.** Il était **code mort** listé dans `CLAUDE.md` et portait un bug : `target = Date.now() + offset` calculait la cible **au montage**, donc chaque visiteur voyait la même durée pour l'éternité et un rechargement la remettait à zéro — le faux compte à rebours d'urgence. Il accepte désormais une **échéance absolue** (`{ until }`) et expose `expired`, qui bascule la page en état « clos ». L'échéance vit dans une constante unique (`GIVEAWAY_CLOSES_AT`) qui pilote le rebours, l'état clos et le texte du règlement.

**Bugs & fix (trouvés au navigateur, pas aux tests).**
- **Images figées à leur hauteur intrinsèque.** Les attributs `width`/`height` réservent le ratio, mais sans `height: auto` la hauteur de l'attribut l'emporte : le héros rendait 461 × **1067** px. Corrigé sur les deux images.
- **Le chapeau de section héritait d'un `text-align: right`.** `.s-head p` attrapait aussi le `<p class="lab">` du premier bloc, qui partait donc se coller à 190 px de la marge. Le paragraphe de droite est maintenant ciblé en enfant direct (`.s-head > p`).
- **`document.title` français avec une espace parasite.** Le titre était recollé depuis les deux lignes du H1 (`titleA` + `titleB`), ce qui donnait « Gagne une paire d' AirPods Max ». La découpe typographique du H1 n'est pas le titre du document : clé `pageTitle` dédiée.
- **Réponses désordonnées.** La connexion d'un compte et la résolution de l'identité arrivent par deux chemins asynchrones ; un jeton de génération ignore désormais la réponse d'un chargement dépassé au lieu de laisser la dernière arrivée gagner.

**Vérifié au navigateur (EN et FR).** Lien du footer → `#/giveaway` (et une seule occurrence sur toute la landing) ; visiteur anonyme → 0 entrée, zéro appel API ; compte lié → 1 ; compte lié + trade paper réel passé contre l'API locale → **4**, avec `/auth/paper/refresh` puis la lecture des badges ; parrainage marqué « bientôt » et non compté ; compte à rebours qui décrémente vers le 19 septembre ; règlement déplié avec les dates localisées ; responsive à 580 px sans débordement horizontal.

**Reste à faire.** Le backend des entrées, spécifié dans [`docs/superpowers/specs/2026-09-09-giveaway-entries-design.md`](superpowers/specs/2026-09-09-giveaway-entries-design.md) : table + store + service + routes + code de parrainage, avec la règle anti-sybil qui compte — **les +2 ne tombent que quand le filleul a fait son premier trade paper**, pas à son inscription. Tant qu'il n'est pas livré, la campagne peut être lancée avec deux règles vivantes sur trois.

## 2026-09-14 — Pitch Make Waves + page publique « Vision & roadmap » `#/roadmap`

**Quoi.** (1) `docs/PITCH.md` : texte de soumission du hackathon (one-liner, description courte ~120 mots, description longue, version courte FR, notes d'usage). (2) Page bilingue `#/roadmap` : `data/roadmap/{types,index}.ts` (contenu `Localized` + `resolveRoadmap`), `views/RoadmapView.vue` (344 l., CSS une règle par ligne), `usePageSeo` dans `useSeo.ts`, route ajoutée à `PUBLIC_ROUTES`, montage lazy dans `App.vue`, lien `linkRoadmap` en dernier de la colonne Ressources du footer landing. 3 fichiers de tests (`roadmap-content`, `roadmap-view`, `roadmap-wiring`, 20 cas) : parité EN/FR non vide, compte des sections, allow-list des chemins de CTA, formules interdites (« risk-free », « sans risque », promesse de gain…), style scoped sans `--gold`/`--up`/`--down`/`--live`/`box-shadow`, restauration des meta au démontage, lien footer, exclusion de l'app-bar. **1209 tests, typecheck 8/8, lint 0, build OK.**

**Pourquoi.** Le pitch de base (« cross-market trading trainer ») annonçait actions et marchés prédictifs que l'app n'a pas : un juge qui ouvre l'app trouverait moins que promis. Repositionnement : **Learn. Prove. Trade.** — Tide est l'école de trading du XRP Ledger, le funnel finance le premier compte XRPL du débutant parce que le produit est pédagogique, que le registre rend ça abordable (2,22 XRP, quasi tout finit dans le compte gardé) et que c'est la première ligne d'un historique on-chain. Le « Hyperliquid de XRPL » est cadré comme la phase 2027, conditionnée à la programmabilité native ; d'ici là : « on forme les traders avant de construire la place de marché ». Aucun chiffre de traction (règle `05-facts.md`).

**Cheminement.** Contenu et contrat DOM écrits par Fable (architecte) ; construction par un workflow Opus 5 : vue + câblage en parallèle des tests, revue adversariale (13 constats), agent de vérification qui applique et fait tourner tests/typecheck/lint/build. Constats retenus notables : `TEST_COUNT` figé faux (1169 → 1209) ; description SEO trop longue → champ `hero.seoDescription` ≤ 160 c. ; test « pas de gold » vacueux (inline styles) → lecture du bloc `<style scoped>` du SFC ; `twitter:card=summary_large_image` retiré (aucune image de partage, décision documentée dans `index.html`).

**Bugs & fix.** `useSeo.ts` : `clear()` ne retirait que les balises *créées* ; les meta statiques d'`index.html` (description, og:*, canonical) écrasées restaient poisonnées pour toute la session après une visite (défaut identique dans `useArticleSeo`, dont le commentaire prétendait le contraire). Fix : `ManagedTag.prev` capture les valeurs avant écriture, `clearManaged()` les restaure ; test de régression ajouté. **Reste** : vérif navigateur (extension Chrome indisponible), commit.

## 2026-09-14 — Roadmap v2 : la page parle à l'équipe XRPL, et le pitch change de cran

**Quoi.** Réécriture complète du contenu de `#/roadmap` (`data/roadmap/{types,index}.ts`), de la vue et des tests, plus `docs/PITCH.md` v2. Nouvelle structure : hero + **thèse** (5 points : le ledger a la liquidité pas les traders ; les autres simulateurs s'arrêtent à la simulation ; on trade en paper ce qu'aucun simulateur n'offre ; la stack est déjà un moteur de traders financés ; humains et agents sur les mêmes rails), **ce qui tourne** (Learn/Prove/Trade), **roadmap** en timeline verticale datée (now/next/then/later), **le DEX** en panneau propre avec badge « pas encore disponible », **bloc ingénieurs XRPL** visuellement délimité (onboarding financé tx par tx, primitives, stack). Sections `problem/funding/xrpl/hackathon` supprimées. **1213 tests, typecheck 8/8, lint 0, build OK.**

**Pourquoi.** Retour d'Eli sur la v1 : trop de « bullshit » et de précautions, pas assez de conviction, et une page qui parlait aux users alors qu'elle est lue par XRPL. Brainstorm produit (regard PM) → trois ajouts structurants validés : (1) les gains de compétition tombent dans le compte XRPL créé par Tide (« le premier vrai XRP, gagné pas acheté ») ; (2) un programme de **traders financés** (évaluation paper sous les règles de risque des mandats, puis capital réel sur le DEX natif) — sans jamais le mot « prop firm », verrouillé par test ; (3) le **DEX en deux temps** (v0 off-chain matching / RLUSD on-chain via multisig, v1 on-chain) pour que « futur Hyperliquid sur XRPL » ait une architecture et pas seulement une date XRPL. Plus le canal éducation (vidéos, workshops, webinaires, coach IA, ligues hebdo, classes, tournois tiers). Retirés : encart honnêteté, conversion du prix en comptes (interdite par Eli), cadences.

**Point de vigilance.** Les **marchés prédictifs** sont affirmés au présent (décision d'Eli, implémentation en cours côté Armand) : à vérifier en ligne avant le 21/09, sinon retirer la mention. Idem « première compétition à cash prize ouverte ».

**Bugs & fix (revue Opus).** `TEST_COUNT` faux (auto-référent, désormais commenté) ; hiérarchie des titres (le CTA final était plus gros que le hero et le DEX) ; `.stage-facts` jamais en une colonne sous 720 px ; « Xaman/GemWallet » rendus comme des transactions XRPL dans la colonne tx (→ `tx: []`, test qui interdit tout nom de wallet dans les chips) ; description SEO FR à 163 caractères (→ 158, test ≤ 160) ; allow-list de routes en dur remplacée par `ROUTES`.
